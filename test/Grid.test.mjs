...
describe("Grid", () => {
  ...
  test('can return grid for encoding', () => {
    let grid = new Grid(2, 2, parsedBlock)
    expect(grid.getGrid()).to.deep.equal(parsedBlock)
  })
});
...
describe('Simulate one generation', () => {
  ...
  test('alive cells with two neighbors survive', () => {
  ...
  test('dead cells with two neigh do not come alive', () => {
  ...
  test('cells with more than 3 neigh die', () => {
    const grid = new Grid(3, 3, [
      [false, true, false],
      [true, true, true],
      [false, true, false]
